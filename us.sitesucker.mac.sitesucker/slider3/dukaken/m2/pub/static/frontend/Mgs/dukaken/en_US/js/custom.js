require([
	...
	(function($) {
		...
		$(document).on("click", ".btn_action_text", function () {
		...
		$(document).ready(function(){
			...
			$(document).on("click", ".menu-fr-sidebar .toogle_menu", function () {
				$('body').addClass('active-menu');
			});
			
			$(document).on("click", ".menu-fr-sidebar .disable_menu", function () {
				$('body').removeClass('active-menu');
				$('.menu-fr-sidebar .nav-megamenu li.level0.active').removeClass('active');
			});
			
			$('.nav-main-menu .mega-menu-item li > span.toggle-menu').click(function(){
			...
			$('.dropdown-submenu .toggle-menu').click(function(){
				$(this).parent().find('.dropdown-menu-ct').slideToggle();
			});
		...
		success: function(response, status) {
			...
			if (status == 'success') {
				if(response.backUrl){
					...
					self.initAjaxAddToCart(tag, 'catalog-add-to-cart-' + $.now(), response.backUrl, formData);
				}else{
					if (response.ui) {
...
function reInitQuickview($, prodUrl, c){
	var ratioImage = 'ratio' + c.attr('data-ratio');
	if (!prodUrl.length) {
		return false;
	}
